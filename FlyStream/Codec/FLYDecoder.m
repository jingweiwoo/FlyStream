...
#import "FLYVideoRGBFrame.h"
#import "FLYVideoYUVFrame.h"
...
- (BOOL)openURL:(NSString *)url error:(NSError **)error {
    if (url == nil) {
        [FLYUtil createError:error
                  ...
                  andMessage:[FLYUtil localizedString:@"Invalid URL"]];
        return NO;
    }
    ...
    if (ret != 0) {
        ...
        [FLYUtil createError:error
                  withDomain:FLYErrorDomainDecoder
                     andCode:ErrorCodeCannotOpenInput
                  andMessage:[FLYUtil localizedString:@"Cannot open input content."]];
        return NO;
    }
    ...
    if (ret < 0) {
        ...
        [FLYUtil createError:error
                  withDomain:FLYErrorDomainDecoder
                     andCode:ErrorCodeCannotFindStreamInfo
                  andMessage:[FLYUtil localizedString:@"Cannot find stream info"]];
        return NO;
    }
    ...
    if (vstream >= 0 && vcodectx != NULL) {
        ...
        [FLYDecoder stream:fmtctx->streams[vstream] fps:&_videoFPS timebase:&_videoTimebase default:0.04];
    ...
    if (astream >= 0 && acodectx != NULL) {
        ...
        [FLYDecoder stream:fmtctx->streams[astream] fps:NULL timebase:&_audioTimebase default:0.025];
    ...
    if (vstream < 0 && astream < 0) {
        ...
        [FLYUtil createError:error
                         withDomain:FLYErrorDomainDecoder
                            andCode:ErrorCodeNoVideoOrAudioStream
                         andMessage:[FLYUtil localizedString:@"No video or audio stream"]];
        return NO;
    }
...
- (NSArray<FLYVideoFrame *> *)handlePicturePacket:(AVPacket *)packet {
    ...
    NSMutableArray<FLYVideoFrame *> *frames = [NSMutableArray array];
    FLYVideoRGBFrame *frame = [[FLYVideoRGBFrame alloc] init];
...
- (NSArray<FLYVideoFrame *> *)handleVideoPacket:(AVPacket *)packet byContext:(AVCodecContext *)context andFrame:(AVFrame *)frame andSwsContext:(struct SwsContext *)swsctx andSwsFrame:(AVFrame *)swsframe {
    ...
    do {
        ...
        if (_isYUV) {
            FLYVideoYUVFrame *yuv = [[FLYVideoYUVFrame alloc] init];
            yuv.Y = [FLYDecoder dataFromVideoFrame:frame->data[0]
                                                linesize:frame->linesize[0]
                                                   width:width
                                                  height:height];
            yuv.Cb = [FLYDecoder dataFromVideoFrame:frame->data[1]
                                                 linesize:frame->linesize[1]
                                                    width:width / 2
                                                   height:height / 2];
            yuv.Cr = [FLYDecoder dataFromVideoFrame:frame->data[2]
            ...
            FLYVideoRGBFrame *rgb = [[FLYVideoRGBFrame alloc] init];
...
- (NSArray<FLYAudioFrame *> *)handleAudioPacket:(AVPacket *)packet byContext:(AVCodecContext *)context andFrame:(AVFrame *)frame andSwrContext:(SwrContext *)swrctx andSwrBuffer:(void **)swrbuf andSwrBufferSize:(int *)swrbufsize {
    int ret = avcodec_send_packet(context, packet);
    if (ret != 0) { NSLog(@"avcodec_send_packet: %d", ret); return nil; }
    
    NSMutableArray<FLYAudioFrame *> *frames = [NSMutableArray array];
    do {
        ret = avcodec_receive_frame(context, frame);
        if (ret == AVERROR_EOF || ret == AVERROR(EAGAIN)) { break; }
        else if (ret < 0) { NSLog(@"avcodec_receive_frame: %d", ret); break; }
       ...
       FLYAudioFrame *f = [[FLYAudioFrame alloc] init];
        f.data = mdata;
